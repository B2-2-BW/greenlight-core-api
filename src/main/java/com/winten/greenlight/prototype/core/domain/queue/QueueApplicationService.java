...
public class QueueApplicationService {
     ...
     */
        public Mono<EntryTicket> checkOrEnterQueue(Long actionId, String destinationUrl, String greenlightToken, Map<String, String> requestParams, long entryTimestamp) {
...
//                }
                ...
                return cachedActionService.getActionGroupById(action.getActionGroupId())
                    .flatMap(actionGroup -> {
                        if (!actionGroup.getEnabled()) {
                            return Mono.just(new EntryTicket(action.getId(), null, destinationUrl, entryTimestamp, WaitStatus.DISABLED, null));
                        }
                            ...
                            return tokenDomainService.getActionIdFromToken(greenlightToken)
                                .flatMap(tokenActionId -> {
                                    if (actionId.equals(tokenActionId)) {
                                        ...
                                        return Mono.just(new EntryTicket(action.getId(), tokenDomainService.extractCustomerId(greenlightToken), destinationUrl, entryTimestamp, WaitStatus.READY, greenlightToken));
                                    } else {
                                        // 1-1. 만약 이전 action의 토큰이라면, waiting queue에 저장하는 로직 태운다.
                                        // 기존 토큰이 유효하지만 actionId가 다른 경우, 기존 customerId를 재사용하여 READY 상태로 처리
                                        return Mono.just(new EntryTicket(action.getId(), tokenDomainService.extractCustomerId(greenlightToken), destinationUrl, entryTimestamp, WaitStatus.READY, greenlightToken));
                                    }
                                })
                                .switchIfEmpty(handleNewEntry(actionId, action.getActionGroupId(), destinationUrl, action, requestParams, entryTimestamp))
                                .onErrorResume(e -> {
                                    // 토큰에서 actionId 추출 실패 시 신규 진입자로 처리
                                    return handleNewEntry(actionId, action.getActionGroupId(), destinationUrl, action, requestParams, entryTimestamp);
                                });
                        } else {
                            // 4. 토큰이 없는 신규 진입자 처리
                                                                return handleNewEntry(actionId, action.getActionGroupId(), destinationUrl, action, requestParams, entryTimestamp);
     ...
     * @return Mono<EntryTicket> 대기 상태 및 토큰 정보
     */
    private Mono<EntryTicket> handleNewEntry(Long actionId, Long actionGroupId, String destinationUrl, Action action, Map<String, String> requestParams, long entryTimestamp) {
                ...
                queueDomainService.isWaitingRequired(actionGroupId)
                    .flatMap(isWaiting -> {
                        if (isWaiting) {
                            ...
                            return tokenDomainService.issueToken(customerId, action, WaitStatus.WAITING.name(), destinationUrl, entryTimestamp)
                                .flatMap(newJwt ->
                                    queueDomainService.addUserToQueue(actionGroupId, customerId, WaitStatus.WAITING, entryTimestamp)
                                            .then(actionEventPublisher.publish(WaitStatus.WAITING, actionGroupId, actionId, customerId, entryTimestamp))
                                            .thenReturn(new EntryTicket(action.getId(), customerId, destinationUrl, entryTimestamp, WaitStatus.WAITING, newJwt))
                                );
                        } else {
                            // 6. 대기가 필요 없는 경우: READY 토큰 발급 및 준비열 등록
                            return tokenDomainService.issueToken(customerId, action, WaitStatus.READY.name(), destinationUrl, entryTimestamp)
                                .flatMap(newJwt ->
                                    queueDomainService.addUserToQueue(actionGroupId, customerId, WaitStatus.READY, entryTimestamp)
                                        .thenReturn(new EntryTicket(action.getId(), customerId, destinationUrl, entryTimestamp, WaitStatus.READY, newJwt))
                                );
                        }
                    })
            );
